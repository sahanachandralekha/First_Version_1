...
export function INAIAvatar({ state = "idle", gesture, mouthOpenness = 0, gaze = "center", size = "md", intensity = 1, signSequence, speech = "" }: INAIAvatarProps) {
  const [poseFailed, setPoseFailed] = useState(false); const [mouthFailed, setMouthFailed] = useState(false);
  ...
  return <>
    <motion.div role="img" aria-label={labels[state]} className={`relative isolate overflow-hidden rounded-full bg-primary-tint ${sizeClasses[size]}`} animate={{ scale: poseFailed ? 1 : [1, 1.015, 1] }} transition={{ duration: 4, repeat: Infinity }}>
      {!poseFailed ? <img className="absolute inset-0 size-full object-contain" src={driver.getPose(renderState)} alt="" onError={() => setPoseFailed(true)} /> : <div className="absolute inset-0 grid place-items-center"><div className="grid size-3/5 place-items-center rounded-full border-4 border-primary/20 bg-background text-center text-sm font-bold text-primary"><Sparkles aria-hidden="true" className="mb-1 size-8"/><span>INAI</span><span className="text-[10px] text-muted-foreground">approved sprites pending</span></div></div>}
      {!poseFailed && !mouthFailed && <img className="absolute inset-0 size-full object-contain" src={driver.getMouth(mouthOpenness)} alt="" onError={() => setMouthFailed(true)} />}
      {state === "speaking" && poseFailed && <span className="absolute bottom-4 left-1/2 size-3 -translate-x-1/2 animate-pulse rounded-full bg-live" aria-hidden="true" />}
    </motion.div>
